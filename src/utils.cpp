...
void get_lens_model(std::vector<std::vector<double>> &front_points, std::vector<std::vector<double>> &back_points, std::vector<std::vector<int>> &triangles, double thickness, double width, double height, int res_x, int res_y, const std::string& filename) {
    std::vector<std::vector<double>> model_vertices;
    std::vector<std::vector<double>> model_triangles;
    ...
    // Curved mesh verts on the bottom
    for (const auto& point : front_points) {
        model_vertices.push_back({width - point[0], height - point[1], -(point[2])});
        //file << "v " << width - point[0] << " " << height - point[1] << " " << -(point[2]) << "\n";
    }
